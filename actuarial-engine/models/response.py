...
class IFRS17Response(BaseModel):
    ...
    aggregate_metrics: AggregateMetrics = Field(..., description="Aggregate portfolio metrics")
    assumptions_used: Dict[str, Any] = Field(..., description="Assumptions used in calculation")
    calculation_timestamp: str = Field(..., description="Calculation timestamp")
    execution_time: Optional[float] = Field(None, description="Calculation execution time in seconds")
...
class SolvencyResponse(BaseModel):
    ...
    scr_breakdown: Dict[str, float] = Field(..., description="SCR breakdown by risk category")
...
class CalculationStatus(BaseModel):
    ...
    progress: int = Field(..., ge=0, le=100, description="Calculation progress percentage")
