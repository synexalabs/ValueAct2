...
class PolicyData(BaseModel):
    ...
    maturity_date: Optional[str] = Field(
        None, description="Policy maturity date (ISO format)"
    )
    ...
    participation_factor: Optional[float] = Field(
        None, ge=0, le=1, description="Insurer's share of underlying items (VFA)"
    )
...
class IFRS17Assumptions(BaseModel):
    ...
    expense_inflation: float = Field(
        ..., ge=0, le=0.1, description="Expense inflation rate (0-0.1)"
    )
    risk_adjustment_factor: Optional[float] = Field(
        0.02, ge=0, le=0.1, description="Risk adjustment factor"
    )
    expense_loading: Optional[float] = Field(
        0.05, ge=0, le=0.2, description="Expense loading factor"
    )
    tax_rate: Optional[float] = Field(0.25, ge=0, le=0.5, description="Tax rate")
    use_dynamic_lapse: Optional[bool] = Field(
        True, description="Use dynamic duration-based lapse rates"
    )
    locked_in_rates: Optional[Dict[str, float]] = Field(
        None, description="Historical locked-in rates by cohort year"
    )
    ...
    def validate_mortality_table(cls, v: str) -> str:
        ...
        if not (is_dav or is_legacy):
            raise ValueError(
                f'Mortality table "{v}" is not supported. Use a DAV table (e.g., DAV_2008_T) or CSO_2017.'
            )
        return v
...
class SolvencyAssumptions(BaseModel):
    ...
    confidence_level: float = Field(
        ..., ge=0.95, le=0.999, description="Confidence level (0.95-0.999)"
    )
    ...
    underwriting_risk_factor: float = Field(
        ..., ge=0, le=1, description="Underwriting risk factor"
    )
    operational_risk_factor: float = Field(
        ..., ge=0, le=1, description="Operational risk factor"
    )
    diversification_benefit: Optional[float] = Field(
        0.15, ge=0, le=0.5, description="Diversification benefit"
    )
    # New fields for equity symmetric adjustment
    current_equity_index: Optional[float] = Field(
        None, description="Current equity index level"
    )
    reference_equity_index: Optional[float] = Field(
        None, description="3-year reference equity index level"
    )
    equity_type: Optional[str] = Field(
        "type_1", description="Equity type for shock calculation"
    )
...
class IFRS17Request(BaseModel):
    ...
    policies: List[PolicyData] = Field(
        ..., min_length=1, description="List of policies"
    )
