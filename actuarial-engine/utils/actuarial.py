...
def validate_assumptions(
    assumptions: Dict[str, Any], calculation_type: str
) -> Dict[str, Any]:
    """
    ...
    if calculation_type == "IFRS17":
        ...
        if "mortality_table" not in assumptions:
            ...
            errors.append(
                f"Ungültige Sterbetafel: {assumptions['mortality_table']}. Verfügbar: {', '.join(VALID_TABLES)}"
            )
...
def calculate_annuity_factor(
    age: int, mortality_table: Dict[str, Any], discount_rate: float, term: int = None
) -> float:
    """
...
def calculate_risk_adjustment(
    portfolio_data: pd.DataFrame, assumptions: Dict[str, Any]
) -> float:
    """
...
def calculate_cohort_metrics(
    cohort_policies: List[Dict[str, Any]], assumptions: Dict[str, Any]
) -> Dict[str, float]:
    """
...
def calculate_csm_release_pattern(
    portfolio_csm: float, assumptions: Dict[str, Any]
) -> List[float]:
    """
