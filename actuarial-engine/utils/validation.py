...
class ReasonablenessValidationRule(ValidationRule):
    ...
    def validate(self, actual_value: float) -> bool:
        ...
        if self.method == "percentage":
            deviation = abs(actual_value - self.benchmark_value) / abs(
...
class ValidationEngine:
    ...
    def add_reasonableness_test(
        ...
        rule = ReasonablenessValidationRule(
            name, description, benchmark_value, tolerance, method, severity
        )
        self.reasonableness_tests.append(rule)
        return rule

    def validate_csm_calculation(
        self, premium: float, fcf: float, ra: float, csm: float
    ) -> Dict[str, Any]:
        ...
        self.add_range_check(
            "CSM Non-Negative", "CSM cannot be negative", min_value=0, severity="error"
        )
        ...
        def csm_formula_check(premium, fcf, ra, csm):
            ...
            return (
                abs(csm - expected_csm_1) < tolerance
                or abs(csm - expected_csm_2) < tolerance
            )
        ...
        def onerous_contract_check(premium, fcf, ra, csm):
            ...
            if is_onerous:
                # Should be zero for onerous contracts
                return csm == 0 or abs(csm) < 1e-6
            return True
    ...
    def validate_solvency_calculation(
        self, scr: float, mcr: float, available_capital: float
    ) -> Dict[str, Any]:
        ...
        # Range checks
        self.add_range_check(
            "SCR Positive", "SCR must be positive", min_value=0, severity="error"
        )

        self.add_range_check(
            "MCR Positive", "MCR must be positive", min_value=0, severity="error"
        )
    ...
    def validate_present_value_calculation(
        self, cash_flows: List[float], discount_rate: float, present_value: float
    ) -> Dict[str, Any]:
        ...
        def pv_formula_check(cash_flows, discount_rate, present_value):
            ...
            expected_pv = sum(
                cf / (1 + discount_rate) ** (i + 1) for i, cf in enumerate(cash_flows)
            )
    ...
    def get_validation_results(self) -> Dict[str, Any]:
        ...
        all_passed = (
            all(check["passed"] for check in self.validation_results["rangeChecks"])
            and all(
                check["passed"]
                for check in self.validation_results["consistencyChecks"]
            )
            and all(
                test["passed"]
                for test in self.validation_results["reasonablenessTests"]
            )
        )
        ...
        logger.info(
            f"Validation completed. Overall status: {'PASSED' if all_passed else 'FAILED'}"
        )
...
def validate_csm_results(
    premium: float, fcf: float, ra: float, csm: float
) -> Dict[str, Any]:
...
def validate_solvency_results(
    scr: float, mcr: float, available_capital: float
) -> Dict[str, Any]:
...
def validate_present_value_results(
    cash_flows: List[float], discount_rate: float, present_value: float
) -> Dict[str, Any]:
    """Convenience function to validate present value calculation results"""
    engine = ValidationEngine()
    return engine.validate_present_value_calculation(
        cash_flows, discount_rate, present_value
    )
...
if __name__ == "__main__":
    ...
    csm_results = validate_csm_results(premium=100000, fcf=80000, ra=10000, csm=10000)
    ...
    solvency_results = validate_solvency_results(
        scr=5000000, mcr=2000000, available_capital=8000000
    )
    ...
    pv_results = validate_present_value_results(
        cash_flows=[10000, 10000, 10000], discount_rate=0.05, present_value=27232.48
    )
