...
class AuditLogger:
    ...
    def add_formula_used(
        self, formula_id: str, version: str, name: str, latex: str
    ) -> None:
    ...
    def add_assumption_used(
        self, name: str, value: Any, source: str, justification: Optional[str] = None
    ) -> None:
        ...
        self.add_execution_log(
            "info", f"Assumption used: {name} = {value} (source: {source})"
        )
    ...
    def add_validation_result(
        ...
        level = (
            ...
            else "error" if not passed else "info"
        )
        self.add_execution_log(
            level, f"Validation {check_name}: {'PASSED' if passed else 'FAILED'}"
        )
    ...
    def add_execution_log(
        ...
        log_entry = ExecutionLog(
            level=level, message=message, data=data, timestamp=datetime.now()
        )
    ...
    def get_audit_trail(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "intermediateResults": [
                asdict(result) for result in self.intermediate_results
            ],
            "methodologyVersion": self.methodology_version,
            "formulasUsed": [asdict(formula) for formula in self.formulas_used],
            "assumptionsUsed": [
                asdict(assumption) for assumption in self.assumptions_used
            ],
            "validationResults": [
                asdict(validation) for validation in self.validation_results
            ],
    ...
    def get_validation_summary(self) -> Dict[str, Any]:
        ...
        consistency_checks = [
            v for v in self.validation_results if v.type == "consistency"
        ]
        reasonableness_tests = [
            v for v in self.validation_results if v.type == "reasonableness"
        ]
    ...
    def finish_calculation(self) -> None:
        ...
        self.add_execution_log(
            "info", f"Calculation completed in {execution_time:.2f} seconds"
        )
...
def validate_range(
    value: float, min_value: float, max_value: float, variable_name: str
) -> bool:
