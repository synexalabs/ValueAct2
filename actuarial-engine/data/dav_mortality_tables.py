...
def _create_unisex_table(
    male_table: dict, female_table: dict, male_weight: float = 0.5
) -> dict:
    ...
    for age in male_table["rates"]:
        ...
        unisex_rates[age] = round(
            male_rate * male_weight + female_rate * female_weight, 3
        )
    return {
        "name": male_table["name"]
        .replace("Male", "Unisex")
        .replace("Female", "Unisex"),
        "description": male_table["description"]
        .replace("Männer", "Unisex")
        .replace("Frauen", "Unisex")
        + " (50/50 Mischung gem. EU-Richtlinie 2004/113/EG)",
...
def get_dav_mortality_rate(
    table_id: str, age: int, gender: str = None, year: int = 2024
) -> float:
    """
    ...
    if "2004_R" in table_id and "trend" in table:
        ...
        if capped_age in trend_data:
            ...
            if not lower_ages:
                ...
                trend_factor = l_val + (u_val - l_val) * (capped_age - l_age) / (
                    u_age - l_age
                )
...
def get_dav_survival_probability(
...
def get_dav_life_expectancy(
    table_id: str, age: int, gender: str = None, max_age: int = 121
) -> float:
    """
