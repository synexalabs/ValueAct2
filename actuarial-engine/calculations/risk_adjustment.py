...
def calculate_projected_scr(
    portfolio_scr: float, projection_years: int, runoff_pattern: str = "linear"
) -> List[float]:
    """
    ...
    if runoff_pattern == "linear":
        ...
        return [
            portfolio_scr * (1 - t / projection_years) for t in range(projection_years)
        ]
    elif runoff_pattern == "exponential":
        # Exponential decay with 10% annual decrease
        decay_rate = 0.10
        return [
            portfolio_scr * np.exp(-decay_rate * t) for t in range(projection_years)
        ]
...
def calculate_risk_adjustment_portfolio(
    portfolio_data: pd.DataFrame, assumptions: Dict[str, Any], method: str = "coc"
) -> Dict[str, Any]:
    """
    ...
    coc_rate = assumptions.get(
        "cost_of_capital_rate", COC_RATES.get(jurisdiction, 0.06)
    )
    discount_rate = assumptions.get("discount_rate", 0.035)

    # Calculate total FCF and variability
    total_fcf = (
        portfolio_data.get("fcf", pd.Series([0])).sum()
        if "fcf" in portfolio_data.columns
        else 0
    )
    ...
    if method == "coc":
        ...
        # Project SCR
        projected_scr = calculate_projected_scr(
            estimated_scr, avg_term, "coverage_units"
        )
        ...
        total_ra = calculate_risk_adjustment_confidence_level(
            fcf_mean=abs(total_fcf), fcf_std=fcf_std, confidence_level=confidence_level
        )
    else:
    ...
    for _, row in portfolio_data.iterrows():
        allocation = (
            (row["face_amount"] / total_face) * total_ra if total_face > 0 else 0
        )
        ra_by_policy.append(
            {
