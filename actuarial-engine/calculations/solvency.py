...
from utils.regulatory_constants import (
    ...
    PROPERTY_SHOCK,
    CURRENCY_SHOCK,
    get_equity_shock_with_symmetric_adjustment,
    get_interest_rate_shock,
)
...
def calculate_portfolio_scr(
    ...
    health_underwriting_risk_scr = calculate_health_underwriting_risk_scr(
        df, assumptions
    )
    non_life_underwriting_risk_scr = calculate_non_life_underwriting_risk_scr(
        df, assumptions
    )
    ...
    # Assume 150% solvency ratio
    own_funds = assumptions.get("own_funds", total_scr_adjusted * 1.5)
    ...
    for _, row in df.iterrows():
        ...
        # 15% shock to mortality (Art. 137)
        life_underwriting_shock = row["face_amount"] * 0.15
    ...
    results = {
        ...
        "policy_type_breakdown": calculate_scr_by_policy_type(
            policies, {**assumptions, "total_scr": total_scr_adjusted}
        ),
        "age_group_breakdown": calculate_scr_by_age_group(
            policies, {**assumptions, "total_scr": total_scr_adjusted}
        ),
...
def calculate_interest_rate_risk(
    ...
    average_duration = assumptions.get(
        "average_duration", 10
    )  # Default 10 year duration
...
def calculate_property_risk(df: pd.DataFrame, assumptions: Dict[str, Any]) -> float:
    ...
    property_exposure = assumptions.get(
        "property_exposure", 0.05
...
def calculate_currency_risk(df: pd.DataFrame, assumptions: Dict[str, Any]) -> float:
    ...
    currency_exposure = assumptions.get(
...
def calculate_concentration_risk(
    ...
    total_assets = df["face_amount"].sum()
    concentration_factor = assumptions.get(
...
def calculate_counterparty_risk_scr(
    df: pd.DataFrame, assumptions: Dict[str, Any]
) -> float:
    """
...
def calculate_life_underwriting_risk_scr(
    df: pd.DataFrame, assumptions: Dict[str, Any]
) -> float:
    """
...
def calculate_disability_risk(df: pd.DataFrame, assumptions: Dict[str, Any]) -> float:
    ...
    disability_exposure = assumptions.get(
        "disability_exposure", 0.1
    )  # 10% disability exposure
    total_assets = df["face_amount"].sum()
...
def calculate_lapse_risk(df: pd.DataFrame, assumptions: Dict[str, Any]) -> float:
    ...
    mass_lapse_shock = assumptions.get(
        "mass_lapse_shock", LAPSE_SHOCKS["mass_lapse"]
    )  # 40%
    lapse_up_shock = assumptions.get("lapse_up_shock", LAPSE_SHOCKS["up_shock"])  # 50%
    lapse_down_shock = assumptions.get(
        "lapse_down_shock", LAPSE_SHOCKS["down_shock"]
    )  # 50%
...
def calculate_revision_risk(df: pd.DataFrame, assumptions: Dict[str, Any]) -> float:
    ...
    revision_exposure = assumptions.get(
        "revision_exposure", 0.05
    )  # 5% revision exposure
    total_assets = df["face_amount"].sum()
...
def calculate_catastrophe_risk(df: pd.DataFrame, assumptions: Dict[str, Any]) -> float:
    ...
    total_benefits = df["face_amount"].sum()
    catastrophe_factor = assumptions.get(
        "catastrophe_factor", 0.001
    )  # 0.1% catastrophe risk

    return total_benefits * catastrophe_factor


def calculate_health_underwriting_risk_scr(
    df: pd.DataFrame, assumptions: Dict[str, Any]
) -> float:
...
def calculate_non_life_underwriting_risk_scr(
    df: pd.DataFrame, assumptions: Dict[str, Any]
) -> float:
    """Calculate non-life underwriting risk SCR"""
    non_life_exposure = assumptions.get(
        "non_life_exposure", 0.05
    )  # 5% non-life exposure
    total_assets = df["face_amount"].sum()
...
def calculate_operational_risk_scr(
    df: pd.DataFrame, assumptions: Dict[str, Any]
) -> float:
    """
    ...
    technical_provisions = assumptions.get(
        "technical_provisions", df["face_amount"].sum() * 0.95
    )
...
def calculate_correlated_scr(
    risk_components: Dict[str, float], risk_type: str
) -> float:
    """
    ...
    for i, risk1 in enumerate(risk_names):
        for j, risk2 in enumerate(risk_names[i + 1 :], i + 1):
            ...
            correlation_terms.append(
                2 * correlation * risk_components[risk1] * risk_components[risk2]
            )
...
def calculate_total_scr(scr_components: Dict[str, float]) -> float:
    ...
    correlations = {
        ...
        # Correct: 0.50 not 0.25
        ("counterparty_risk", "non_life_underwriting_risk"): 0.50,
    ...
    for i, risk1 in enumerate(risk_names):
        for j, risk2 in enumerate(risk_names[i + 1 :], i + 1):
            ...
            correlation_terms.append(
                2 * correlation * scr_components[risk1] * scr_components[risk2]
            )
...
def calculate_mcr(df: pd.DataFrame, assumptions: Dict[str, Any]) -> float:
    ...
    technical_provisions_guaranteed = assumptions.get(
        "tp_guaranteed", df["face_amount"].sum() * 0.80
    )
    technical_provisions_other = assumptions.get(
        "tp_other", df["face_amount"].sum() * 0.15
    )
    capital_at_risk = assumptions.get("capital_at_risk", df["face_amount"].sum() * 0.50)

    mcr_linear = (
        MCR_LINEAR_FACTORS["life"]["technical_provisions_guaranteed"]
        * technical_provisions_guaranteed
        + MCR_LINEAR_FACTORS["life"]["technical_provisions_other"]
        * technical_provisions_other
...
def calculate_scr_by_policy_type(
    policies: List[Dict[str, Any]], assumptions: Dict[str, Any]
) -> Dict[str, float]:
    """
...
def calculate_scr_by_age_group(
    policies: List[Dict[str, Any]], assumptions: Dict[str, Any]
) -> Dict[str, float]:
    """
...
def calculate_scr_sensitivity(
    ...
    for factor_change in [-0.05, -0.02, 0.02, 0.05]:
        ...
        modified_assumptions["market_risk_factor"] = (
            modified_assumptions.get("market_risk_factor", 0.25) + factor_change
        )
        modified_scr = calculate_portfolio_scr(policies, modified_assumptions)["scr"]
        sensitivity_results[f"market_risk_factor_{factor_change}"] = (
            modified_scr - base_scr
        )

    # Interest rate shock sensitivity
    for shock_change in [-0.005, -0.002, 0.002, 0.005]:
        modified_assumptions = base_assumptions.copy()
        modified_assumptions["interest_rate_shock"] = (
            modified_assumptions.get("interest_rate_shock", 0.01) + shock_change
        )
        modified_scr = calculate_portfolio_scr(policies, modified_assumptions)["scr"]
        sensitivity_results[f"interest_rate_shock_{shock_change}"] = (
            modified_scr - base_scr
        )
...
def calculate_scr_stress_tests(
    policies: List[Dict[str, Any]], base_assumptions: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    # 200bp interest rate shock
    market_crash_assumptions["interest_rate_shock"] = 0.02
    stress_tests["market_crash"] = calculate_portfolio_scr(
        policies, market_crash_assumptions
    )["scr"]
    ...
    stress_tests["mortality_catastrophe"] = calculate_portfolio_scr(
        policies, mortality_cat_assumptions
    )["scr"]
    ...
    stress_tests["combined_stress"] = calculate_portfolio_scr(
        policies, combined_stress_assumptions
    )["scr"]
...
def calculate_lac_dt(df: pd.DataFrame, assumptions: Dict[str, Any]) -> float:
    ...
    lac_dt = min(deferred_tax_assets * tax_rate, base_scr * max_lac_dt_factor)
